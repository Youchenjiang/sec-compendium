...
- **數位鑑識取證資料集 (Forensics Evidence)**：已統整收錄於 [`security/practice/exams/evidence/`](../exams/evidence/)（專供模擬測驗取證實作）。
- **HITCON 實戰攻防與 Wargame 案例庫**：已統整收錄於 [`tracks/lab/hitcon_range/wargame_case_study/`](../../../tracks/lab/hitcon_range/wargame_case_study/README.md)（含自動化漏洞挖掘框架與覆盤戰報）。
- **90-Runs 實戰衝刺檢核表**：已統整收錄於 [`tracks/lab/90_runs/checklists/`](../../../tracks/lab/90_runs/checklists/)（含 PortSwigger、picoCTF 與雙平台綜合檢核表）。
- **攻防實驗室教室環境手冊**：已統整收錄於 [`tracks/lab/90_runs/cylab_classroom_setup_guide.md`](../../../tracks/lab/90_runs/cylab_classroom_setup_guide.md)。
